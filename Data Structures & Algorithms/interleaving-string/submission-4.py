class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        # ALGORITHM STEPS: INTERLEAVING STRING (BOTTOM-UP DP)
        #
        # 1. SUBPROBLEMS: Check if suffix s1[i:] and suffix s2[j:] interleave to form s3[i+j:]
        # 2. STATE:       dp[i][j] is True if s1[i:] and s2[j:] interleave to form s3[i+j:]
        # 3. TRANSITION:  If s1[i] matches s3[i+j], check advancing s1 pointer (dp[i+1][j])
        #                 If s2[j] matches s3[i+j], check advancing s2 pointer (dp[i][j+1])
        #                 dp[i][j] is True if either choice yields a valid path
        # 4. ITERATION:   Loop i backwards (len1 down to 0), j backwards (len2 down to 0)
        # 5. BASE:        dp[len1][len2] = True (empty s1 and s2 form empty s3 suffix)
        # 6. RESULT:      dp[0][0] shows if full s1 and s2 interleave to form full s3
