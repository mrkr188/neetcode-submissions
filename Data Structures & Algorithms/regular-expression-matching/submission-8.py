class Solution:
    def isMatch(self, s: str, p: str) -> bool:
        ...
        # 1. STATE: dp[r][c] = True if suffix s[r:] matches pattern suffix p[c:]
        # 2. BASE CASE: dp[ROWS][COLS] = True (empty s matches empty p)
        # 3. ITERATION: r from ROWS down to 0, c from COLS - 1 down to 0
        # 4. MATCH CHECK: match = r < ROWS and (s[r] == p[c] or p[c] == '.')
        # 5. STAR STATE TRANSITION (if p[c+1] == '*'):
        #    dp[r][c] = dp[r][c+2] (skip 'x*') or (match and dp[r+1][c] -> keep star)
        # 6. STANDARD STATE TRANSITION (no star):
        #    dp[r][c] = dp[r+1][c+1] if match else False
        # 7. RESULT: dp[0][0] represents full s[0:] matching full p[0:]
